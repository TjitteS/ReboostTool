...
MPPTInterface::MPPTInterface(QObject *parent) : QObject(parent)
{
    mmpptConfig = new ConfigParams(this);
    ...
    mCommands->setmpptConfig(mmpptConfig);
    ...
    connect(mmpptConfig, SIGNAL(updated()), this, SLOT(bmsconfUpdated()));
    connect(mmpptConfig, SIGNAL(stored()), this, SLOT(bmsconfStored()));
}

MPPTInterface::~MPPTInterface()
{
...
Commands *MPPTInterface::commands() const
{
    return mCommands;
}

ConfigParams *MPPTInterface::mpptConfig()
{
    return mmpptConfig;
}

ConfigParams *MPPTInterface::infoConfig()
{
    return mInfoConfig;
}

QStringList MPPTInterface::getSupportedFirmwares()
{
...
QList<QPair<int, int> > MPPTInterface::getSupportedFirmwarePairs()
{
...
QString MPPTInterface::getFirmwareNow()
{
    return mFwTxt;
}

void MPPTInterface::emitStatusMessage(const QString &msg, bool isGood)
{
    emit statusMessage(msg, isGood);
}

void MPPTInterface::emitMessageDialog(const QString &title, const QString &msg, bool isGood, bool richText)
{
    emit messageDialog(title, msg, isGood, richText);
}

bool MPPTInterface::fwRx()
{
    return mFwVersionReceived;
}

BleUart *MPPTInterface::bleDevice()
{
    return mBleUart;
}

void MPPTInterface::storeBleName(QString address, QString name)
{
    mBleNames.insert(address, name);
}

QString MPPTInterface::getBleName(QString address)
{
    QString res;
...
bool MPPTInterface::isPortConnected()
{
    bool res = false;
...
void MPPTInterface::disconnectPort()
{
#ifdef HAS_SERIALPORT
    if(mSerialPort->isOpen()) {
        mSerialPort->close();
...
bool MPPTInterface::reconnectLastPort()
{
...
bool MPPTInterface::autoconnect()
{
    bool res = false;
...
QString MPPTInterface::getConnectedPortName()
{
...
bool MPPTInterface::connectSerial(QString port, int baudrate)
{
#ifdef HAS_SERIALPORT
...
QList<VSerialInfo_t> MPPTInterface::listSerialPorts()
{
...
void MPPTInterface::connectTcp(QString server, int port)
{
...
void MPPTInterface::connectBle(QString address)
{
...
bool MPPTInterface::isAutoconnectOngoing() const
{
    return mAutoconnectOngoing;
}

double MPPTInterface::getAutoconnectProgress() const
{
    return mAutoconnectProgress;
}

#ifdef HAS_SERIALPORT
void MPPTInterface::serialDataAvailable()
{
...
void MPPTInterface::serialPortError(QSerialPort::SerialPortError error)
{
    QString message;
    switch (error) {
...
void MPPTInterface::tcpInputConnected()
{
    mTcpConnected = true;
    updateFwRx(false);
}

void MPPTInterface::tcpInputDisconnected()
{
    mTcpConnected = false;
    updateFwRx(false);
}

void MPPTInterface::tcpInputDataAvailable()
{
...
void MPPTInterface::tcpInputError(QAbstractSocket::SocketError socketError)
{
    (void)socketError;
...
void MPPTInterface::bleDataRx(QByteArray data)
{
    mPacket->processData(data);
}

void MPPTInterface::timerSlot()
{
...
void MPPTInterface::packetDataToSend(QByteArray &data)
{
#ifdef HAS_SERIALPORT
    if (mSerialPort->isOpen()) {
...
void MPPTInterface::packetReceived(QByteArray &data)
{
    mCommands->processPacket(data);
}

void MPPTInterface::cmdDataToSend(QByteArray &data)
{
    mPacket->sendPacket(data);
}

void MPPTInterface::fwVersionReceived(int major, int minor, QString hw, QByteArray uuid)
{
    QList<QPair<int, int> > fwPairs = getSupportedFirmwarePairs();
...
void MPPTInterface::updateFwRx(bool fwRx)
{
...
void MPPTInterface::setLastConnectionType(conn_t type)
{
    mLastConnType = type;
